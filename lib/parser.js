...
parsers.repoInfo = function(output) {
    ...
    for (i = 1; i < match.length - 1; i++) {
        var branchWithStatus = match[i].split(/pushes to /);

        name = branchWithStatus[1].substr(0, branchWithStatus[1].indexOf(' '))
        status = branchWithStatus[1].substr(branchWithStatus[1].indexOf(' ') + 1);
