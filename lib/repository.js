...
var debug = require('debug')('git-controller:repository');
...
///TODO: add async ones
/**
 * Performs a checkout to new branch intended to validate a Pull Request
 * #checkoutPullRequest
 * @param {string} tempbranch
 * @param {string} startingpoint
 */
Repository.prototype.checkoutPullRequestSync = function(tempbranch, startingpoint) {
    var self = this;
    debug('executing checkout pullrequest');
    debug(tempbranch);
    debug(startingpoint);
    var checkoutCommand = tempbranch +  (startingpoint !== undefined ? (' ' + startingpoint) : '');
    debug(checkoutCommand);
    var cmd = new Command(self.path,'checkout', ['-b '], checkoutCommand);
 ...
 * @param {string} startingpoint
 */
Repository.prototype.destroyBranchForcedSync = function(tempbranch, startingpoint) {
    var self = this;
    debug('executing forced branch destroyed ');
    
    if (! startingpoint) throw "you need a branch to set/checkout to";
    
    debug('will destroy branch ' + tempbranch);
    debug('after checking out ' + startingpoint);
    
    self.checkoutSync(startingpoint);
    var cmd = new Command(self.path,'branch', ['-D'], tempbranch);
    debug(cmd.command);
    return cmd.execSync();
};
 ...
 * @param {string} startingpoint
 */
Repository.prototype.pullRebaseOursSync = function(headUrl, headRef) {
    var self = this;
    
    if (! headUrl) throw "need the head Url to rebase from";
    if (! headRef) throw "need the head Ref to rebase from"

    debug('headUrl: ' + headUrl);
    debug('headRef: ' + headRef);    
    
    var cmd = new Command(self.path,'pull', ['--rebase', '-s', 'ours', '--ff-only'], headUrl + ' ' + headRef);
    debug(cmd.command);
    return cmd.execSync();
};
